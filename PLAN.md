...
打开我们的网页（控制面板）
  → 点击"开始翻译"
  → 浏览器弹窗 → 用户选择正在播放视频的标签页
  → getDisplayMedia() 捕获该标签的音频
  → 实时语音识别 + 翻译
  → 画中画浮动字幕窗口弹出 → 始终置顶，显示中英字幕
```
...
| 模式 | 用户操作 | 技术实现 |
|------|---------|---------|
| **标签页捕获**（主力） | 点击开始 → 选择视频标签页 | `getDisplayMedia()` 捕获标签音频 |
| **文件上传**（辅助） | 上传本地 mp4/mp3/wav 文件 | `<video>` / `<audio>` 播放 |
...
│   ├── pip.html            # 画中画字幕窗口页面
│   ├── style.css           # 样式
│   └── app.js              # 前端逻辑（音频捕获 + WebSocket + PiP）
├── docs/
...
WebSocket 返回翻译结果到前端
        ↓
BroadcastChannel 广播到画中画字幕窗口
        ↓
画中画窗口实时显示中英双语字幕
        ↓
final 到达时 → 自动修正 interim 翻译
```
